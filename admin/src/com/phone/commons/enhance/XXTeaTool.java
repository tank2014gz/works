package com.phone.commons.enhance;
...
public class XXTeaTool {
	private static final Logger logger = Logger.getLogger(XXTeaTool.class);
	...
	public static String encrypt(String plain, String charset, String hexKey)
		...
		return ByteFormat.toHex(encrypt(plain.getBytes(charset), ByteFormat
				.hexToBytes(hexKey)));
	}
	...
	public static String decrypt(String cipherHex, String charset, String hexKey) {
		...
		try {
			return new String(decrypt(ByteFormat.hexToBytes(cipherHex),
					ByteFormat.hexToBytes(hexKey)), charset);
		} catch (Exception ex) {
			logger.warn(ex, ex);
		}
	...
	public static byte[] encrypt(byte[] plainData, byte[] key) {
		...
		return toByteArray(encrypt(toIntArray(plainData, true), toIntArray(key,
				false)), false);
	}
	...
	public static byte[] decrypt(byte[] cipherData, byte[] key) {
		...
		return toByteArray(decrypt(toIntArray(cipherData, false), toIntArray(
				key, false)), true);
	}
